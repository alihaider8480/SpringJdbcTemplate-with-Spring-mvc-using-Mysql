...
public class EmployeeDAOImpl implements EmployeeDAO
{
    ...
    */
	...
	{
	  jdbcTemplate.update("DELETE FROM employee_table WHERE employee_Id=?",emplyoee_id);	
	}

	@Override
	public String updateemployeedata(EmployeePojo employeepojoobj)
	{
	   jdbcTemplate.update("UPDATE employee_table set email=?,employee_name=?,salary=?,email=?,gender=? WHERE employee_Id=?",employeepojoobj.getEmail(),employeepojoobj.getEmploeeName(),employeepojoobj.getSalary(),employeepojoobj.getEmail(),employeepojoobj.getGender(),employeepojoobj.getEmployeeId() );		   
	...
	{
		return jdbcTemplate.query("select * From employee_table WHERE employee_Id=?", new EmployeeRowMapper(), id);
	}
